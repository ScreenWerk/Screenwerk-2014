...
do
  ...
  d_H="$((`echo $start_time|cut -c1-2`-`date +'%H'`))"
  d_M="$((`echo $start_time|cut -c4-5`-`date +'%M'`))"
  d_S="$((`echo $start_time|cut -c7-8`-`date +'%S'`))"
#  d_NS=$((`echo $start_time|cut -c10-11`-`date --rfc-3339=ns|cut -d. -f2|cut -c1-2`))
  echo ${start_time} - `date --rfc-3339=ns`
...
#/usr/bin/mplayer -monitoraspect 1280/800 -geometry 321:241 /mnt/swshare/154327.flv -loop 0 2>&1 >/dev/null &
