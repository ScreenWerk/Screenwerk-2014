...
// var consoleStream = fs.createWriteStream('./c.log.info', {flags:'a'})
...
var SwPlayer = function SwPlayer(err, dom_element, callback) {
    if (err) {
        c.log.error('SwPlayer err:', err, dom_element.swElement)
        callback(err)
    }
    // c.log.info('SwPlayer:', dom_element.swElement)
    ...
    if (element.definition.keyname !== 'sw-media') {
        ...
        for (var key=0; key<dom_element.childNodes.length; key++) {
            ...
            // c.log.info(typeof child_node)
            // c.log.info(child_node.id)
            child_node.player = new SwPlayer(null, child_node, callback)
        }
    }
    ...
    return {
        play: function(err, timeout, callback) {
            if (err) {
                c.log.error('SwPlayer.play err:', err, element.id)
                return this
            }
            // c.log.info('Attempting PLAY on ' + element.definition.keyname + ' ' + element.id)
            if (properties['valid-from'] !== undefined) {
                if (properties['valid-from'].values !== undefined) {
                    ...
                    if (vf_date.getTime() > Date.now()) {
                        c.log.info('DOM id: ' + element.id + ' valid-from not reached.', vf_date.getTime())
            ...
            // c.log.info(dom_element.id + ' PLAY ' + element.definition.keyname + ' ', is_playing ? '(Already playing)' : '(Was stopped)')
            if (is_playing === true) { return this }
            is_playing = true
            ...
            switch (element.definition.keyname) {
                ...
                case 'sw-configuration':
                    var schedule_nodes = []
                    // c.log.info(Object.keys(dom_element.childNodes))
                    // TODO: consider forEach
                    // dom_element.childNodes.forEach(function(child_node) {
                ...
                case 'sw-schedule':
                    ...
                    if (properties.cleanup.values[0].db_value === 1) {
                        ...
                        for (key=0; key<dom_element.parentNode.childNodes.length; key++) {
                            ...
                            // c.log.info((schedule,{depth:6}))
                            var sibling_node_cleanup_layer = sibling_node.swElement.properties.ordinal.values[0].db_value
                            // c.log.info('Schedule ' + (element.id)
                            ...
                            if (sibling_node_cleanup_layer <= cleanupLayer) {
                                // c.log.info('|-- Schedule ' + element.id + ' cleaning up schedule ' + sibling_node.swElement.id)
                                sibling_node.player.stop()
                            }
                        }
                    }
                    if (properties.duration.values !== undefined) {
                        ...
                        // c.log.info(helper.msToTime(time_from_start) + ' passed, ' + helper.msToTime(time_to_play) + ' left.')
                ...
                case 'sw-media':
                    ...
                    if (mediatype === 'Video') {
                        ...
                        try {
                            ...
                            c.log.error('WARNING: Media DOM element has no play() function.', e)
                            process.exit(99)
                        }
        ...
        stop: function(err, timeout, callback) {
            ...
            if (timeout && timeout > 0) {
                ...
                // tcIncr()
                // helper.swLog('timeout_counter: ' + timeout_counter)
                // c.log.info(dom_element.id + ' Scheduling STOP on ' + element.definition.keyname + ' ' + element.id + ' in ' + helper.msToTime(timeout), 'Timeouts set: ' + timeout_counter)
            ...
            // c.log.info(dom_element.id + ' STOP ' + element.definition.keyname, is_playing ? '(Was playing)' : '(Already stopped)')
            if (is_playing === false)
                return this
            is_playing = false
                ...
                case 'sw-media':
                    ...
                    if (mediatype === 'Video') {
                        ...
                        try {
                            ...
                            c.log.error('WARNING: Media DOM element has no pause() function.', e)
                            process.exit(99)
                        }
                    } else if (mediatype === 'URL') {
                        document.getElementById('if' + element.id).contentWindow.postMessage(
        ...
        clearMyTimeouts: function(err) {
            // c.log.info('Clearing ' + my_timeouts.length + ' my_timeouts.', 'Timeouts set total: ' + timeout_counter)
            while (my_timeouts.length > 0) {
                clearTimeout(my_timeouts.pop())
            }
        },
        restart: function(err, callback) {
            c.log.info('RESTART ' + element.id, 'Current state: ' + (is_playing ? 'playing' : 'stopped'))
            // document.getElementById('progress').style.visibility = 'hidden'
            document.getElementById('progress').style.display = 'none'
            return this.stop(null, 0, function(err, data) {
                if (err) {
                    c.log.error('SwPlayer.restart err:', err, data)
                    callback(err, data)
                }
...
function clearSwTimeouts() {
    c.log.info('Clearing ' + sw_timeouts.length + ' sw_timeouts.', 'Timeouts set total: ' + timeout_counter)
