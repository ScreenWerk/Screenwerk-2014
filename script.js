...
var SwPlayer    = require('./player.js')
...
if (uuids.length === 1) {
    ...
    if (typeof(gui.App.argv[0]) !== 'string' ) {
        alert("Screen ID should be passed as first argument or screen_id.uuid file should be in users home folder.")
        process.exit(1)
    }
    if (Number(gui.App.argv[0]) <= 0) {
        alert("Screen ID must be number greater than zero.")
        process.exit(1)
    }
...
if (fs.existsSync(uuid_path)) {
    ...
    c.__API_KEY = uuid.v1()
    alert('Created key for screen: ' + c.__SCREEN_ID + '(' + uuid_path + '). Now register this key in Entu: ' + c.__API_KEY)
    process.exit(0)
}

console.log(c.__API_KEY.toString())
...
function startDigester(err, data) {
    ...
    if (loader.loading_process_count > 0) {
    ...
    function flushMeta(err) {
        ...
        loader.swElements.every(function(swElement, idx) {
            if (swElement.definition.keyname !== 'sw-media' && swElement.childs.length === 0) {
                ...
                loader.unregisterMeta(null, idx, function(err, data) {
                    if (err) {
